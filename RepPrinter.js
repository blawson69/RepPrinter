...
var RepPrinter = RepPrinter || (function () {
    ...
    handleInput = function (msg) {
        if (msg.type == 'api' && msg.content.startsWith('!print')) {
			...
			if (parms.length == 3) {
				var char_id = parms[2];
				if (parms[1].match(/^(?:equipment|spell[s0-9]{1}|utility|offense|ammo|skills|armor|class)$/i) == null) {
					var valid_categories = _.map(_.keys(repstrings), function(x) {return x;});
                    showDialog('Error', '"' + parms[1] + '" is not a valid category. Valid categories are equipment, spells, utility, offense, ammo, skills, armor, and class.<br><br>You may also narrow the spells down by using "spell" plus the spell level with no spaces. Use spell0 for cantrips.<br><br>Please try again.');
				} else {
