...
var RepPrinter = RepPrinter || (function () {
    ...
    var version = '0.1',
		repstrings = {equipment: '^(?:repeating_equipment_).+$', utility: '^(?:repeating_utility_).+$', offense: '^(?:repeating_offense_).+$', ammo: '^(?:repeating_ammo_).+$', spells: '^(?:repeating_spell).+$', spell0: '^(?:repeating_spell0).+$', spell1: '^(?:repeating_spell1).+$', spell2: '^(?:repeating_spell2).+$', spell3: '^(?:repeating_spell3).+$', spell4: '^(?:repeating_spell4).+$', spell5: '^(?:repeating_spell5).+$', spell6: '^(?:repeating_spell6).+$', spell7: '^(?:repeating_spell7).+$', spell8: '^(?:repeating_spell8).+$', spell9: '^(?:repeating_spell9).+$', skills: '^(?:repeating_skill_).+$', armor: '^(?:repeating_armor_).+$', class: '^(?:repeating_class_).+$', modifiers: '^(?:repeating_modifier_).+$', attachers: '^(?:repeating_attacher_).+$', classFeature: '^(?:repeating_classfeature_).+$', random: '^.*REPLACE.*$'},
        styles = {
    ...
    handleInput = function (msg) {
        if (msg.type == 'api' && msg.content.startsWith('!print')) {
			...
			if (parms.length > 2) {
				var char_id = parms[1];
				if (parms[2].match(/^(?:equipment|spell[s0-9]{1}|utility|offense|ammo|skills|armor|class|modifiers|attachers|classfeature|random)$/i) == null) {
